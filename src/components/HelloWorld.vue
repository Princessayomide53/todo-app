<template>
  <div class="bg-purple-200 h-[80rem] px-3 md:px-0">
    <h1 class="text-3xl leading-normal pt-7">Todo List</h1>
    <form @submit.prevent="addTodo" class="space-x-4 md:space-x-7 inline-block">
      <input
...
export default {
  ...
  setup() {
    ...
    const { result, loading } = useQuery(GET_TODOS);
    ...
    const { mutate: deleteTodoMutation } = useMutation(DELETE_TODO, {
    ...
    const deleteTodo = async (id) => {
      ...
      await deleteTodoMutation(variables)
        .then(() => {
